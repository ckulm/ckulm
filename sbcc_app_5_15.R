...
Game_File <- read.csv("~/Desktop/sbcc/sbcc_data_5_02.csv") %>% 
  filter(PitcherTeam == "SAN_BAR1") %>% 
...
BACON <- read.csv("~/Desktop/sbcc/xwOBAcon.csv")
umpire_data <- read_csv("~/Desktop/sbcc/called_strike.csv")
...
FinalCSV$Date <- mdy(FinalCSV$Date)
